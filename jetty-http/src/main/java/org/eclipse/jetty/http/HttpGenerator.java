...
public class HttpGenerator
{
    ...
    private static final Set<String> __assumedContentMethods = new HashSet<>(Arrays.asList(new String[]{HttpMethod.POST.asString(),HttpMethod.PUT.asString()}));
                            ...
                            for (int i=0;i<values.length;i++)
                            {
                                ...
                                switch (value==null?HttpHeaderValue.UNKNOWN:value)
                                {
                                    ...
                                    case CLOSE:
                                    {
                                        close=true;
                                        _persistent=false;
                                        if (response!=null && _endOfContent == EndOfContent.UNKNOWN_CONTENT) {
											_endOfContent=EndOfContent.EOF_CONTENT;
										}
                                        break;
                                    }
    ...
    private void putContentLength(ByteBuffer header, long contentLength, boolean contentType, MetaData.Request request, MetaData.Response response)
    {
        ...
        else if (!_noContent && (contentType || response!=null || (request!=null && __assumedContentMethods.contains(request.getMethod())))) {
